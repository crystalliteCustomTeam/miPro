...
class BasicController extends Controller
{
    ...
    function payment_edit_amount_process(Request $request, $id)
    {
        ...
        } elseif ($request->input('paymentNature') == "FSRemaining") {
            $transactionType = "New Lead";
        } else {
            $transactionType = $request->input('paymentNature');
        }
    ...
    function csv_sheetpayments_processinfinity(Request $request)
    {
        ini_set('max_execution_time', 300);

        $a =  json_encode(["--"]);

        $data = Excel::toArray([], $request->file('infinitysheetpayments'));
        $allinvoice = [];
        foreach ($data as $extractData) {
            $headings = $extractData[0];
            $keycount = count($headings);
        ...
        foreach ($allinvoice as $allinvoices) {
            ...
            $infinityemail = $allinvoices[0]['Email'];
            $findclient = Client::where('email', $infinityemail)->count();
            $findclient1 = Clientmeta::whereJsonContains('otheremail', $infinityemail)->count();
            $totalemailcount = (int)$findclient + (int)$findclient1;
            if ($totalemailcount > 0) {
                continue;
            } else {
                $infinitybrand = Brand::where("name", $allinvoices[0]['Brand'])->get();
                $infinityseller = Employee::where("name", $allinvoices[0]['Sales Person'])->get();
                $createClient = Client::insertGetId([
                    'name' => $allinvoices[0]['Card Name'],
                    'phone' => 0000,
                    'email' => $allinvoices[0]['Email'],
                    'brand' => $infinitybrand[0]->id,
                    'frontSeler' => $infinityseller[0]->id,
                    'website' => $allinvoices[0]['URL'],
                    'created_at' => date('y-m-d H:m:s'),
                    'updated_at' => date('y-m-d H:m:s')
                ]);
            }
        ...
        foreach ($allinvoice as $allinvoices) {
            ...
            $matchclientmeta = Client::where('email', $allinvoices[0]['Email'])->get();
            ...
            if ($matchclientmeta->isNotEmpty()) {
                $findclient = Client::where('id', $matchclientmeta[0]->id)->get();
                $project = Project::where('clientID', $findclient[0]->id)->get();
                if (isset($project[0]->id)) {
                    $findproject = $project[0]->id;
                } else {
